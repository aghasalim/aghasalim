...
Eleven repositories, five with live demos. The table is the index; the write-ups below
give the reasoning, the failures and the numbers I would not want a reviewer to find
on their own.
...
| **MLOps pipeline** | Production pipeline with the monitoring deliberately broken, to prove the drift detector actually fires | [code](https://github.com/aghasalim/mlops-fraud-pipeline) |
| **A/B testing & causal inference** | Estimators checked against simulations with known ground-truth effects, so the method is validated before it meets real data | [code](https://github.com/aghasalim/ab-causal) |
...
| **LoRA fine-tuning** | Fine-tuning with the catastrophic-forgetting check most tutorials skip | [code](https://github.com/aghasalim/lora-forgetting) |
| **Offline vs online metrics** | Recommender where offline ranking gains and online lift disagree, and the write-up explains why | [code](https://github.com/aghasalim/recsys-offline-online) |
